...
import {
  getUserAcademyById,
  updateUserAcademyAction,
} from "@/app/[locale]/my-academies/actions";
import { AcademyForm } from "@/components/AcademyForm";
import { Link, redirect } from "@/i18n/navigation";
import { getAuthUser } from "@/lib/supabase/server";

interface EditUserAcademyPageProps {
  params: Promise<{ locale: string; id: string }>;
}

export default async function EditUserAcademyPage({
  params,
}: EditUserAcademyPageProps) {
  ...
  const academy = await getUserAcademyById(id);
  if (!academy) notFound();
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-8 sm:px-6 lg:px-8">
      ...
      <AcademyForm
        academy={academy}
        action={updateUserAcademyAction}
        successHref="/my-academies"
